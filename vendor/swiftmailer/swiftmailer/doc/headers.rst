...
Headers are retrieved through the HeaderSet's ``get()`` and ``get()``
methods::
...
insensitive) that is passed to it. When using ``get()`` with a header name,
an array of headers with that name are returned. Calling ``get()`` with no
...
.. note::
    ...
    the Received header). For this reason ``get()`` exists to fetch all
