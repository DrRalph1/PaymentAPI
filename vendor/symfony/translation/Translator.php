...
EOF
            ,
            $locale,
            var_export($this->getMessages($this->catalogues[$locale]), true),
            $fallbackContent
        );
...
EOF
                ...
                $fallbackSuffix,
                $fallback,
                var_export($this->getMessages($fallbackCatalogue), true),
                $currentSuffix,
                $fallbackSuffix
            );
    ...
    private function loadFallbackCatalogues(string $locale): void
    {
        ...
        foreach ($this->computeFallbackLocales($locale) as $fallback) {
            ...
            $fallbackCatalogue = new MessageCatalogue($fallback, $this->getMessages($this->catalogues[$fallback]));
    ...
    private function getMessages(MessageCatalogueInterface $catalogue): array
    {
        $allMessages = [];
